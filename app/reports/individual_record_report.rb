class IndividualRecordReport < BaseReport
  ...
  def self.build(entity_configuration, teacher, year, start_at, end_at, individual_frequencies, student_enrollments, events, school_calendar)
    new(:landscape).build(entity_configuration, teacher, year, start_at, end_at, individual_frequencies, student_enrollments, events, school_calendar)
  end

  def build(entity_configuration, teacher, year, start_at, end_at, individual_frequencies, student_enrollments, events, school_calendar)
  ...
  def header
    ...
    entity_organ_and_unity_cell = make_cell(content: "#{entity_name}\n#{organ_name}\n#{@individual_frequencies.first.unity.name}", size: 12, leading: 1.5, align: :center, valign: :center, rowspan: 4, padding: [6, 0, 8, 0])
    ...
    classroom_cell = make_cell(content: @individual_frequencies.first.classroom.description, size: 10, borders: [:bottom, :left, :right], padding: [0, 2, 4, 4], height: 4, colspan: 2)
    ...
    discipline_cell = make_cell(content: (@individual_frequencies.first.discipline ? @individual_frequencies.first.discipline.description : 'Geral'), size: 10, colspan: 3, borders: [:bottom, :left, :right], padding: [0, 2, 4, 4], height: 4)
  ...
  def individual_frequencies_table
    self.any_student_with_dependence = false

    individual_frequencies = @individual_frequencies.reject { |individual_frequency| !individual_frequency.students.any? }

    frequencies_and_events = individual_frequencies.to_a + @events.to_a

    frequencies_and_events = frequencies_and_events.sort_by do |obj|
      individual_frequency?(obj) ? obj.frequency_date : obj[:date]
    end
    ...
    sliced_frequencies_and_events.each_with_index do |frequencies_and_events_slice, index|
      ...
      frequencies_and_events_slice.each do |individual_frequency_or_event|
        if individual_frequency?(individual_frequency_or_event)
          individual_frequency = individual_frequency_or_event
          next unless frequency_in_period(individual_frequency)

          class_numbers << make_cell(content: "#{individual_frequency.class_number}", background_color: 'FFFFFF', align: :center)
          days << make_cell(content: "#{individual_frequency.frequency_date.day}", background_color: 'FFFFFF', align: :center)
          months << make_cell(content: "#{individual_frequency.frequency_date.month}", background_color: 'FFFFFF', align: :center)
          @students_enrollments.each do |student_enrollment|
            student_id = student_enrollment.student_id
            if exempted_from_discipline?(student_enrollment, individual_frequency)
              student_frequency = ExemptedIndividualFrequencyStudent.new
            else
              student_frequency = individual_frequency.students.select{ |student| student.student_id == student_id && student.active == true }.first
              student_frequency ||= NullIndividualFrequencyStudent.new
            end
            student = student_enrollment.student
            (students[student_id] ||= {})[:name] = student.to_s
            students[student_id] = {} if students[student_id].nil?
            students[student_id][:dependence] = students[student_id][:dependence] || student_has_dependence?(student_enrollment, individual_frequency.discipline_id)
          ...
          school_calendar_event = individual_frequency_or_event
  ...
  def individual_frequency?(record)
    record.is_a? IndividualFrequency
  end
  ...
  def exempted_from_discipline?(student_enrollment, individual_frequency)
    return false unless individual_frequency.discipline_id.present?

    discipline_id = individual_frequency.discipline_id
    step_number = step_number(individual_frequency)
  ...
  def step_number(individual_frequency)
    @steps ||= StepsFetcher.new(individual_frequency.classroom).steps

    step = @steps.select{ |step| step[:start_at] <= individual_frequency.frequency_date && step[:end_at] >= individual_frequency.frequency_date }.first

    step.to_number unless step.nil?
  end

  def frequency_in_period(individual_frequency)
    step_number(individual_frequency).present?
  end
end
